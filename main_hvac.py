...
print("Ví dụ: Máy lạnh không mát, dàn nóng chạy nhưng gió yếu, có tiếng kêu lạ")
...
try:
    final_answer = None
    for chunk in hvac_app.stream({"task": task}, config, stream_mode="values"):
        if "messages" in chunk and chunk["messages"]:
            last_msg = chunk["messages"][-1]
            if isinstance(last_msg, tuple):
                role = last_msg[0].replace("_", " ").title()
                content = str(last_msg[1])[:300] + "..." if len(str(last_msg[1])) > 300 else last_msg[1]
